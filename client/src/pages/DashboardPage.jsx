...
const TIER_CONFIG = {
  ...
  developing:     { label: 'Developing',      bg: 'bg-neutral-50/80',   text: 'text-neutral-500',   border: 'border-neutral-200/60', barBg: 'bg-neutral-200',   icon: Shield, dot: 'bg-neutral-300' },
...
function CommissionDashboard({ period }) {
  ...
  if (!data || !data.leaderboard) return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-neutral-800">Commission Dashboard</h1>
      <div className="card p-12 text-center">
        <Trophy className="w-12 h-12 text-neutral-200 mx-auto mb-3" />
        <h3 className="text-lg font-medium text-neutral-600 mb-1">No Commission Data</h3>
        <p className="text-neutral-400">Run calculations for this period to see the leaderboard</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-neutral-800">Commission Dashboard</h1>
        <p className="text-sm text-neutral-400 mt-1">Performance leaderboard & incentive analytics</p>
      </div>
      ...
      {/* Podium */}
      {top3.length >= 3 && (
        <div className="card p-6 bg-gradient-to-br from-neutral-50 via-white to-violet-50/30 border border-neutral-100">
          <h3 className="font-semibold text-neutral-700 mb-5 flex items-center gap-2">
            ...
            <button
              ...
              className={cn(
                ...
                selectedKpi === kpi.id
                  ...
                  : 'bg-white text-neutral-500 border border-neutral-200 hover:border-violet-300'
              )}
            >
              {kpi.name}
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card border border-neutral-100">
            ...
            <div className="divide-y divide-neutral-50 max-h-[560px] overflow-y-auto">
              ...
              {filteredLeaderboard.length === 0 && (
                <div className="p-8 text-center text-neutral-300">No data for this filter</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {/* Performance Tiers */}
          <div className="card p-5 border border-neutral-100">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    expandedPlan === plan.plan_id
                      ...
                      : 'border-neutral-100 hover:border-violet-200 hover:bg-neutral-50/50'
                  )}
                  onClick={() => setExpandedPlan(expandedPlan === plan.plan_id ? null : plan.plan_id)}
                >
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-neutral-700 truncate">{plan.plan_name}</span>
                    <ChevronRight className={cn('w-4 h-4 text-neutral-300 transition-transform', expandedPlan === plan.plan_id && 'rotate-90')} />
                  </div>
                  <div className="flex items-center gap-4 mt-1 text-xs text-neutral-400">
                    <span>Base: {formatCurrency(plan.base_payout)}</span>
                    <span>Total: <span className="font-semibold text-neutral-600">{formatCurrency(plan.total_payout)}</span></span>
                  </div>
                  {expandedPlan === plan.plan_id && (
                        ...
                        .map(e => {
                          ...
                          return (
                            <div key={e.employee_id} className="flex justify-between text-xs">
                              <span className="text-neutral-500">{e.employee_name}</span>
                              <span className="font-medium text-neutral-700">{formatCurrency(planPayout?.net_payout || 0)}</span>
                            </div>
                          );
                        })
                      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-5 border border-neutral-100">
          <h3 className="font-semibold text-neutral-700 mb-4">Payout Distribution</h3>
        ...
        <div className="card p-5 border border-neutral-100">
          <h3 className="font-semibold text-neutral-700 mb-4">Commission by Territory</h3>
          {by_territory.length > 0 ? (
            ...
            <div className="h-[240px] flex items-center justify-center text-neutral-300 text-sm">No territory data</div>
          )}
        </div>
      </div>
    </div>
...
function PodiumCard({ rank, emp, highlight }) {
  const colors = {
    ...
    2: { bg: 'from-neutral-50 to-gray-50', border: 'border-neutral-200/60', pedestal: 'bg-neutral-200', emoji: '🥈', h: 'h-20' },
  ...
  return (
    <div className="flex-1 max-w-[180px]">
      <div className={cn(
        ...
        c.bg, c.border,
        highlight && 'shadow-md shadow-amber-100/50 -tranneutral-y-2'
      )}>
        <div className="text-xl mb-1">{c.emoji}</div>
        <div className="w-10 h-10 rounded-full bg-white/80 border mx-auto mb-2 flex items-center justify-center text-xs font-bold text-neutral-600"
          style={{ borderColor: rank === 1 ? '#fbbf24' : rank === 2 ? '#94a3b8' : '#fb923c' }}>
          {initials}
        </div>
        <p className="text-sm font-semibold text-neutral-700 truncate">{emp.employee_name.split(' ')[0]}</p>
        <p className="text-[11px] text-neutral-400 mb-1">{emp.territory || ''}</p>
        <p className="text-base font-bold text-neutral-800">{formatCurrency(emp.total_payout)}</p>
        <p className="text-[11px] text-neutral-400">{formatPercent(emp.avg_achievement)} avg</p>
      </div>
...
function LeaderboardRow({ emp, maxPayout }) {
  ...
  return (
    <div className="px-5 py-3 flex items-center gap-3 hover:bg-neutral-50/50 transition-colors">
      <div className="w-7 text-center">
        {emp.rank <= 3 ? (
          <span className="text-base">{['', '🥇', '🥈', '🥉'][emp.rank]}</span>
        ) : (
          <span className="text-xs font-semibold text-neutral-300">#{emp.rank}</span>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-neutral-700 truncate">{emp.employee_name}</span>
          <span className={cn('text-[10px] px-1.5 py-0.5 rounded-full font-medium', cfg.bg, cfg.text)}>
            {cfg.label}
          </span>
        </div>
        <div className="flex items-center gap-3 mt-1">
          <div className="flex-1 h-1.5 bg-neutral-100/80 rounded-full overflow-hidden">
            <div className={cn('h-full rounded-full transition-all duration-700', cfg.barBg)} style={{ width: `${barWidth}%` }} />
          </div>
          <span className="text-[10px] text-neutral-400 whitespace-nowrap">{formatPercent(emp.avg_achievement)}</span>
        </div>
      </div>
      ...
      <div className="text-right min-w-[90px]">
        <p className="text-sm font-bold text-neutral-700">{formatCurrency(emp.total_payout)}</p>
        <p className="text-[10px] text-neutral-400">Sales: {formatCurrency(emp.total_sales)}</p>
      </div>
    </div>
  );
}
...
function KpiLeaderboardRow({ emp, rank, kpiUnit }) {
  ...
  const barColor = ach >= 120 ? 'bg-amber-300' : ach >= 100 ? 'bg-teal-300' : ach >= 80 ? 'bg-sky-300' : ach >= 50 ? 'bg-neutral-200' : 'bg-rose-200';
  const initials = emp.employee_name.split(' ').map(n => n[0]).join('').slice(0, 2);
    ...
    <div className="px-5 py-3 flex items-center gap-3 hover:bg-neutral-50/50 transition-colors">
      <div className="w-7 text-center">
        {rank <= 3 ? (
          ...
          <span className="text-xs font-semibold text-neutral-300">#{rank}</span>
        )}
      </div>

      <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold border bg-neutral-50 text-neutral-500 border-neutral-200">
        {initials}
      </div>

      <div className="flex-1 min-w-0">
        <span className="text-sm font-medium text-neutral-700 truncate block">{emp.employee_name}</span>
        <div className="flex items-center gap-3 mt-1">
          <div className="flex-1 h-1.5 bg-neutral-100/80 rounded-full overflow-hidden">
            <div className={cn('h-full rounded-full transition-all duration-700', barColor)} style={{ width: `${Math.min(ach, 150) / 1.5}%` }} />
          </div>
          <span className={cn('text-[11px] font-semibold whitespace-nowrap', ach >= 100 ? 'text-teal-600' : ach >= 80 ? 'text-sky-600' : 'text-neutral-400')}>
            {formatPercent(ach)}
          </span>
        </div>
      </div>
      ...
      <div className="text-right min-w-[100px]">
        ...
        <p className="text-[10px] text-neutral-400">Payout: {formatCurrency(emp._kpiPayout)}</p>
      </div>
    </div>
  );
}
    ...
    <div className={cn('rounded-xl p-4 bg-gradient-to-br border border-white/60 shadow-sm', gradient)}>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-lg font-bold text-neutral-800 leading-tight truncate">{value}</p>
...
function SalespersonDashboard({ employeeId, period, persona }) {
  ...
  if (!data) return (
    <div className="space-y-6">
      ...
      <p className="text-neutral-400 mt-1">Welcome back, {persona.name}</p>
      <div className="card p-12 text-center">
        <Trophy className="w-12 h-12 text-neutral-200 mx-auto mb-3" />
        <h3 className="text-lg font-medium text-neutral-600 mb-1">No Commission Data</h3>
        <p className="text-neutral-400">Run a calculation for this period to see your dashboard</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <GlassCard icon={TrendingDown} label="Return %" value={formatPercent(returnPercent)} gradient={returnPercent > 8 ? 'from-rose-100 to-pink-50' : 'from-neutral-100 to-gray-50'} iconColor={returnPercent > 8 ? 'text-rose-500' : 'text-neutral-400'} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {kpiChartData.length > 0 && (
          <div className="card p-5 border border-neutral-100">
            <h3 className="font-semibold text-neutral-700 mb-4">KPI Achievement</h3>
        ...
        {payoutBreakdown.length > 0 && (
          <div className="card p-5 border border-neutral-100">
            <h3 className="font-semibold text-neutral-700 mb-4">Payout Breakdown</h3>
            <div className="space-y-3">
              {payoutBreakdown.map(item => (
                <div key={item.name} className="flex items-center justify-between py-2 border-b border-neutral-50 last:border-0">
                  <span className="text-sm text-neutral-500">{item.name}</span>
                  <span className={cn('text-sm font-semibold', item.value >= 0 ? 'text-neutral-700' : 'text-rose-400')}>
                    {item.value >= 0 ? '+' : ''}{formatCurrency(item.value)}
                  </span>
                </div>
              ))}
              <div className="flex items-center justify-between pt-2 border-t-2 border-neutral-200">
                <span className="text-sm font-bold text-neutral-700">Net Payout</span>
      ...
      {kpi_results.length > 0 && (
        <div className="card border border-neutral-100">
          <div className="p-5 border-b border-neutral-100">
            <h3 className="font-semibold text-neutral-700">KPI Detail</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-100 bg-neutral-50/50">
                  <th className="text-left py-3 px-5 font-medium text-neutral-500">KPI</th>
                  <th className="text-right py-3 px-4 font-medium text-neutral-500">Target</th>
                  <th className="text-right py-3 px-4 font-medium text-neutral-500">Actual</th>
                  <th className="text-right py-3 px-4 font-medium text-neutral-500">Achievement</th>
                  <th className="text-right py-3 px-4 font-medium text-neutral-500">Weight</th>
                  <th className="text-right py-3 px-4 font-medium text-neutral-500">Payout</th>
                </tr>
              </thead>
              <tbody>
                {kpi_results.map(k => (
                  <tr key={k.kpi_id || k.code} className="border-b border-neutral-50">
                    <td className="py-3 px-5">
                      <div className="font-medium text-neutral-700">{k.kpi_name}</div>
                      <div className="text-[11px] text-neutral-400">{k.category}</div>
                    </td>
                    ...
                    <td className="py-3 px-4 text-right text-neutral-500">{k.weight}%</td>
                    <td className="py-3 px-4 text-right font-semibold text-neutral-700">{formatCurrency(k.weighted_payout)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function DashboardSkeleton() {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        {[1, 2, 3, 4, 5].map(i => (
          <div key={i} className="rounded-xl p-4 bg-neutral-50"><div className="h-14 bg-neutral-100 rounded animate-pulse" /></div>
        ))}
      </div>
      <div className="rounded-xl p-5 h-48 animate-pulse bg-neutral-50" />
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 rounded-xl p-5 h-96 animate-pulse bg-neutral-50" />
        <div className="rounded-xl p-5 h-96 animate-pulse bg-neutral-50" />
      </div>
    </div>
  );
}
